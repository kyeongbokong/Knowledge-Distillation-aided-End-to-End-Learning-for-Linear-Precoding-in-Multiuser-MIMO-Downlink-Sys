...
def Dnn_Decoder_low(x, weights, biases, train_flag):
    # x = tf.reshape(x, [-1, imsize, imsize, channel])
...
for u_ii in range(n_user):
    ...
    Out_encoder_low, _ = Dnn_Encoder(x_user, weights_encoder1, biases_encoder1, train_flag)
...
for pdB_set in range(30, -5, -10):
    ...
    model_path = "./weight_" + str(n_tx) + "_" + str(n_rx) + "_" + str(n_user) + "_" + str(
        L_) + "_Channel_no_hint_same_network_" + str(Bit) + "bit_" + str(pdB_set) + "/1"
    ...
    while step <= training_iters:
        ...
        if step % display_step == 0:
            ...
            for i in range(int(test_num / batch_size)):
                ...
                inst_loss_low = sess.run(cost_low, feed_dict={x: batch_valid_y_train, x_ch: batch_x,
                                                                              PdB_tensor: PdB, train_flag: False,
                                                                              keep_prob: 1})
        ...
        if step % test_step == 0:
            ...
            for i in range(int(test_num / batch_size)):
                ...
                inst_loss_low, w_esti_low_, Out_encoder_low_cat_ = sess.run([cost_low, w_esti_low, Out_encoder_low_cat],
                                                                            feed_dict={x: batch_y_train, x_ch: batch_x,
                                                                                       PdB_tensor: PdB,
                ...
                if i == 0:
                    ...
                    w_esti_final = np.concatenate([w_esti_low_], axis=0)
                    H_test = np.concatenate([H_test, batch_x], axis=0)
                    y_train_final = np.concatenate([batch_y_train], axis=0)
            if best_loss > loss_low_val / float(test_num / batch_size):
                ...
                io.savemat("DNN_result_" + str(n_tx) + "_" + str(n_rx) + "_" + str(n_user) + "_" + str(
                    L_) + "_Channel_no_hint_same_network_" + str(Bit) + "bit_" + str(pdB_set) + "dB.mat",
                           mdict={'H_test': H_test, 'y_train_final': y_train_final, 'w_esti_best': w_esti_best,
                                  'loss': best_test_loss})

        # Display log
        if step % 5000 == 0:
            print("Iter " + str(step) + ", Test Loss low= " + "{:.6f}".format(loss_low / float(test_num / batch_size)))
